...
const WhisperCallUI: React.FC<{ targetUser: User }> = ({ targetUser }) => {
  const { state } = useAppContext();
  ...
  // Note: In a real app, you would also get the remote user's stream via WebRTC.
  // Here we just show a placeholder for the other user.
  const remoteUserPlaceholder = (
      <div className="bg-gray-800 aspect-video rounded-md flex items-center justify-center text-purple-200 text-sm h-full px-4 border-2 border-gray-600 relative">
          <div className="flex flex-col items-center justify-center">
             <span className="font-bold text-gray-400 text-sm">{targetUser.name}</span>
          </div>
          <div className="absolute bottom-1 left-1 bg-black bg-opacity-50 px-2 py-0.5 text-xs rounded">{targetUser.name}</div>
      </div>
  );

  return (
    <header className="w-full bg-purple-900 bg-opacity-90 backdrop-blur-sm p-2 shadow-lg z-20 border-b border-purple-700 flex items-center justify-between flex-shrink-0">
      <div className="flex items-center gap-3 h-24">
        <h3 className="text-md font-bold text-white pr-4 border-r border-purple-600">Whispering with {targetUser.name}</h3>
